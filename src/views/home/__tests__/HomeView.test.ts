...
describe('ArticleListItem 首页文章卡片语义', () => {
  it('使用统一封面和内容结构承载文章扫读信息', () => {
    ...
    expect(source).toContain('class="article-main-link"')
    expect(source).toContain('class="article-author-chip"')
    expect(source).not.toContain('showAuthorChip')
    expect(source).toContain('{{ article.user.nickname }}')
    expect(source.indexOf('class="article-main-link"')).toBeLessThan(source.indexOf('class="article-author-chip"'))
    expect(source).toContain('class="article-meta-row"')
    expect(source).toContain('class="article-stats"')
    expect(source).toContain('class="article-taxonomy"')
    expect(source).toContain('class="article-tags"')
    expect(source).toContain('class="article-tag-pill"')
    expect(source).toContain('articleTags.slice(0, 1)')
    expect(source).not.toContain('class="article-category-button"')
    expect(source).not.toContain('tabler:category')
    ...
    expect(source).toMatch(/\.article-stats\s*{[\s\S]*margin-left:\s*auto;/)
    expect(source).toContain('class="article-date"')
    expect(source).toContain('covertTimeHowLongAgo(article.createTime)')
    expect(source).not.toContain('date(article.createTime)')
    expect(source).toMatch(/\.article-main-link\s*{[\s\S]*flex:\s*1;/)
    expect(source).toMatch(/\.article-author-chip\s*{[\s\S]*max-width:\s*120px;/)
    expect(source).toMatch(/\.article-cover-link\s*{[\s\S]*flex:\s*0 0 30%;/)
    expect(source).toMatch(/\.article-wrapper\s*{[\s\S]*width:\s*100%;/)
    expect(source).toMatch(/@media \(max-width: 759px\)\s*{[\s\S]*\.article-wrapper\s*{[\s\S]*height:\s*auto;/)
    expect(source).not.toContain('max-width: 58%')
    expect(source).toContain('height: 180px;')
    expect(source).not.toContain('height: 196px;')
